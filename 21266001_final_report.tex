...
\begin{abstract}
    ...
    評価実験では、初級、中級、上級の異なる技能レベルの選手のサーブ動作を分析した。その結果、上級者は特にローディングフェーズでの膝関節の深い屈曲、平均126.23度と、フォロースルーフェーズでの安定した動作制御、肘関節の標準偏差3.62度が特徴的であることが明らかになった。特に注目すべき点として、上級者のフォロースルー時の膝関節角度の標準偏差は2.38度と非常に小さく、安定した動作の再現性の高さを示している。一方、初級者では特にバックスイングフェーズでの肘の屈曲が大きく、平均96.74度、動作の再現性も低く、標準偏差15.55度であることが確認された。また、中級者は上級者と初級者の中間的な特徴を示し、特にバックスイングでの動作の安定性において肘関節標準偏差2.33度と向上していることが確認された。

    システムの実装面では、一般的なスマートフォンによる4K、60fpsでの撮影映像に対して、リアルタイムでの骨格推定とフェーズ分類が可能である。また、外れ値除去とスムージング処理により、撮影環境の変化や一時的な検出エラーに対してもロバストな分析を実現している。従来のモーションキャプチャシステムと比較して、特別な機器やマーカーの装着を必要とせず、通常の練習環境でそのまま利用可能である点が大きな特徴となっている。

    定量的な動作解析では、各フェーズにおける関節角度の統計的な分析に加え、フェーズ間の遷移タイミングや動作の連続性についても評価を行った。特に、ローディングからフォワードスイングへの移行時における膝関節の伸展速度と、肘関節角度の変化パターンの関係性を明らかにした。これにより、効率的なエネルギー伝達メカニズムの定量的な評価が可能となった。

    本研究の成果は、テニス指導現場において、客観的なデータに基づく効率的な技術指導を可能とするものである。特に、各フェーズでの定量的な動作評価は、選手の技術的課題の特定と改善指針の提示に有用である。また、システムの低コストと汎用性は、幅広い練習環境での活用を可能とする。これにより、データに基づいた科学的なテニス指導の普及に貢献することが期待される。
